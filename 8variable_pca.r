...
# Baca data pada sheet "cslarge" dalam file dan simpan dengan nama "cslarge_raw"
...
# Gambarkan distribusi debt berdasarkan dependents seperti pada contoh boxplot income di atas.
ggplot(cslarge_raw, aes(as.factor(dependents), debt)) + 
   geom_boxplot() + xlab("Dependents") + ggtitle("Boxplot Debt Berdasarkan Dependents")

# Pisahkan data untuk training set dan testing set berdasarkan risk rating
...
# Set seed agar sampling bisa direproduksi
set.seed(100)
...
cslarge_train <- do.call("rbind", list(
   cslarge_raw[train1_index,], cslarge_raw[train2_index,], 
   cslarge_raw[train3_index,], cslarge_raw[train4_index,], 
   cslarge_raw[train5_index,]
))

# Menghapus kolom contractcode dan riskrating dari training set
cstrain <- cslarge_train[, -c("contractcode", "riskrating")]

# Menggabungkan hasil sampling masing-masing risk rating ke dalam testing set
cslarge_test <- do.call("rbind", list(
   cslarge_raw[test1_index,], cslarge_raw[test2_index,], 
   cslarge_raw[test3_index,], cslarge_raw[test4_index,], 
   cslarge_raw[test5_index,]
))

# Menghapus kolom contractcode dan riskrating dari testing set
cstest <- cslarge_test[, -c("contractcode", "riskrating")]

# Menghitung korelasi antar variabel dalam data frame
cor_matrix <- cor(cstrain)

# Gambarkan matrik korelasi dengan ggcorrplot
library(ggcorrplot)
ggcorrplot(cor_matrix)

# Lakukan analisa PCA dengan fungsi prcomp() dan simpan output ke dalam pr.out
...
# Gambarkan Principal Component dan risk rating dengan menggunakan autoplot()
library(ggfortify)
autoplot(pr.out, data = cslarge_train, colour = 'riskrating', 
   loadings = TRUE, loadings.label = TRUE, 
   loadings.label.size = 3, scale = 0)

# Gambarkan Principal Component dan risk rating dengan menggunakan fviz_pca_ind()
library(factoextra)
